  ...
  let (<*>) f m =
    ...
    { run = fun input pos more fail succ ->
      let succ0 input0 pos0 more0 f =
        let succ1 input1 pos1 more1 m = State.Lazy (lazy (succ input1 pos1 more1 (f m))) in
  ...
  let lift2 f m1 m2 =
    { run = fun input pos more fail succ ->
      let succ1 input1 pos1 more1 m1 =
        let succ2 input2 pos2 more2 m2 = State.Lazy (lazy (succ input2 pos2 more2 (f m1 m2))) in
